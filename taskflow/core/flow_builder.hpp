...
public:
    ...
    */
    template <typename B, typename E, typename O, typename C, typename P = DefaultPartitioner,
    ...
    Please refer to @ref ParallelTransforms for details.
    */
    template <typename B1, typename E1, typename B2, typename O, typename C, typename P = DefaultPartitioner,
    ...
    */
    ...
    template <typename B1, typename E1, typename B2, typename T, typename BOP_R, typename BOP_T,
...
// Procedure: linearize
inline void FlowBuilder::linearize(std::vector<Task>& keys) { _linearize(keys); }

// Procedure: linearize
inline void FlowBuilder::linearize(std::initializer_list<Task> keys) { _linearize(keys); }
...
// Function: joinable
inline bool Subflow::joinable() const noexcept { return !(_parent->_nstate & NSTATE::JOINED_SUBFLOW); }

// Function: executor
inline Executor& Subflow::executor() noexcept { return _executor; }
...
// Function: retain
inline bool Subflow::retain() const { return _parent->_nstate & NSTATE::RETAIN_SUBFLOW; }
