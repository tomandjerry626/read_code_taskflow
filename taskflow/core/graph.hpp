...
// Constructor
template <typename... Args>
Node::Node(nstate_t nstate, estate_t estate, const TaskParams& params, Topology* topology, Node* parent,
           size_t join_counter, Args&&... args)
    : _nstate{nstate},
      _estate{estate},
      _name{params.name},
      _data{params.data},
...
// Constructor
template <typename... Args>
Node::Node(nstate_t nstate, estate_t estate, const DefaultTaskParams&, Topology* topology, Node* parent,
           size_t join_counter, Args&&... args)
    : _nstate{nstate},
      _estate{estate},
      _topology{topology},
...
*/
// 做完这个插入后，这个图的前驱和后继就已经确定了，最后这个图每个节点的边为[0~num_successors-1]存储后继节点，[num_successors~n-1]存储前驱节点
inline void Node::_precede(Node* v) {
    // std::cout << "当前节点: " << _name << " 加入后继节点: " << v->_name << std::endl;
    _edges.push_back(v);
    // std::cout << "_edges[_num_successors]: " << _edges[_num_successors]->_name
    // << "_edges[_edges.size() - 1]: " << _edges[_edges.size() - 1]->_name << std::endl;
    std::swap(_edges[_num_successors++], _edges[_edges.size() - 1]);  // 交换后继节点和前驱节点
...
inline size_t Node::num_successors() const { return _num_successors; }

// Function: predecessors
inline size_t Node::num_predecessors() const { return _edges.size() - _num_successors; }
...
// Function: name
inline const std::string& Node::name() const { return _name; }
...
inline bool Node::_is_preempted() const { return _nstate & NSTATE::PREEMPTED; }
...
inline void Node::_set_up_join_counter() {
    ...
    /*
    让我们分解这个表达式：
    当 is_cond == true（即前驱是条件任务，弱依赖）时：
    _nstate = (_nstate + 1) | (1 * 0x10000000)
            = (_nstate + 1) | 0x10000000
    _nstate + 1：低 28 位的弱依赖计数器 +1
    | 0x10000000：同时设置 CONDITIONED 标志位，表示"此节点被条件任务依赖"
    当 is_cond == false（即前驱是普通任务，强依赖）时：
    _nstate = (_nstate + 0) | (0 * 0x10000000)
            = _nstate | 0
            = _nstate  // 保持不变
    */
    for (size_t i = _num_successors; i < _edges.size(); i++) {  // 查找当前节点的所有前驱节点.

        bool is_cond = _edges[i]->_is_conditioner();                      // 判断当前前驱节点是否为条件节点.
        _nstate = (_nstate + is_cond) | (is_cond * NSTATE::CONDITIONED);  // weak dependency
        c += !is_cond;                                                    // strong dependency
    }
    // 每个节点的强依赖(不是前驱节点，因为前驱节点包含强依赖的非条件任务和弱依赖的条件任务)是在这里设置的!!!
    _join_counter.store(c, std::memory_order_relaxed);
    /*
    [_set_up_join_counter] 当前node: D, join_counter: 2
    [_set_up_join_counter] 当前node: C, join_counter: 1
    [_set_up_join_counter] 当前node: B, join_counter: 1
    [_set_up_join_counter] 当前node: A, join_counter: 0
    */
    fprintf(stdout, "[%d] [%d] [_set_up_join_counter] 当前node: %s, join_counter: %d\n", getpid(), gettid(),
            this->name().c_str(), c);
}
...
inline void Graph::_erase(Node* node) {
    erase(std::remove_if(begin(), end(), [&](auto& p) { return p.get() == node; }), end());
}
...
Node* Graph::_emplace_back(ArgsT&&... args) {
    push_back(std::make_unique<Node>(
        std::forward<ArgsT>(args)...));  // 调用基类'std::vector<std::unique_ptr<Node>>'中的push_back函数
    return back().get();
}
