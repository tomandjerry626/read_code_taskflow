...
public:
    ...
    */
    explicit Executor(size_t N = 4, std::shared_ptr<WorkerInterface> wix = nullptr);
...
inline Executor::Executor(size_t N, std::shared_ptr<WorkerInterface> wix)
    : _workers(N), _notifier(N), _buffers(N), _worker_interface(std::move(wix)) {
    if (N == 0) {
...
// Destructor
inline Executor::~Executor() { _shutdown(); }
...
inline size_t Executor::num_workers() const noexcept { return _workers.size(); }
...
// Function: num_queues
inline size_t Executor::num_queues() const noexcept { return _workers.size() + _buffers.size(); }
...
inline size_t Executor::num_taskflows() const { return _taskflows.size(); }
...
exploit:
    ...
    while (!stop_predicate()) {
        ...
        explore:
            ...
            t = (vtm < _workers.size()) ? _workers[vtm]._wsq.steal() : _buffers.steal(vtm - _workers.size());
...
inline bool Executor::_explore_task(Worker& w, Node*& t) {
    ...
    const size_t MAX_STEALS = ((num_queues() + 1) << 1);  // ((4 + 2) + 1) * 2 = 14

    // 创建均匀分布的随机数生成器，范围是 [0, num_queues()-1]
    // 用于随机选择下一个受害者队列
    std::uniform_int_distribution<size_t> udist(0, num_queues() - 1);  // [0, 5]

    // fprintf(stdout, "MAX_STEALS: %lu, udist的范围: [%lu, %lu]\n", MAX_STEALS, udist.min(), udist.max());
    ...
    size_t vtm = w._vtm;  // e.g. vtm = 0, 1, 2, 3, 4, 5
    ...
    while (true) {
        ...
        t = (vtm < _workers.size()) ? _workers[vtm]._wsq.steal()              // 从其他工作线程的队列顶部窃取
...
explore_task:
    ...
    // 1. 进入窃取模式：线程 A 首先将自己的角色转变为“窃取者”（thief），并调用 explore_task
    // 尝试从其他线程的队列以及共享队列中窃取任务
    if (_explore_task(w, t) ==
        false) {  // 返回false是线程收到停止信号，跳出循环的标志.不是窃取失败的标志，窃取成功或者失败都会返回true.
        return false;
    }

    // Go exploit the task if we successfully steal one.
    /*
    窃取成功（简单情况）：如果 explore_task 成功找到了一个任务，线程 A
    就重新变回“活跃的”（active）工作线程，并开始执行这个任务。 段落中还补充了一个关键细节：如果线程 A
    之前是系统里最后一个窃取者，那么它在转为活跃状态后，必须负责唤醒另一个休眠的线程来接替它“窃取者”的角色。这确保了只要系统里有活跃的线程，就至少会有一个线程在不断地寻找新任务，从而保持系统的响应性。
    */
    if (t) {
        // 窃取成功，返回true到'_exploit_task'中执行.
        fprintf(stdout, "[%d] [%d] 窃取成功，%lu号工作线程窃取到%s任务\n", getpid(), gettid(), w.id(),
                t->name().c_str());
        return true;
    }
...
inline size_t Executor::num_observers() const noexcept { return _observers.size(); }
...
// 参数：
//   worker - 执行任务的工作线程
//   cache  - 任务缓存（引用），用于存储待执行的任务
//   node   - 新的就绪任务
//
// 逻辑：
//   1. 如果 cache 已有任务：将 cache 中的任务调度到队列
//   2. 将新任务 node 缓存到 cache
//
// 优化效果：
//   - 如果只有一个后继任务：cache 中只有一个任务，直接执行（尾调用）
//   - 如果有多个后继任务：第一个缓存，其余的调度到队列
//   - 避免 push + pop 的开销
//   - 提高 CPU 缓存命中率（任务数据仍在 L1 缓存中）
//
// 示例：
//   假设任务 A 有 3 个后继任务 B、C、D 同时就绪
//   1. _update_cache(worker, cache, B)  -> cache = B
//   2. _update_cache(worker, cache, C)  -> 调度 B 到队列，cache = C
//   3. _update_cache(worker, cache, D)  -> 调度 C 到队列，cache = D
//   4. _invoke() 末尾：通过 goto 直接执行 D（尾调用优化）
//
TF_FORCE_INLINE void Executor::_update_cache(Worker& worker, Node*& cache, Node* node) {
    ...
    if (cache) {
        // ====================================================================
        // cache 已有任务，需要将其调度到队列
        // ====================================================================
        // 为什么要调度？
        //   - cache 只能存储一个任务（用于尾调用优化）
        //   - 如果有多个后继任务就绪，只能缓存一个，其余的必须调度到队列
        //
        // 调度策略：
        //   - 优先放入工作线程的本地队列（_wsq）
        //   - 如果本地队列满，溢出到中心化缓冲区（_buffers）
        //
    ...
    // 将新任务缓存到 cache
    // ========================================================================
    // 这个任务将在 _invoke() 函数末尾通过 TF_INVOKE_CONTINUATION() 宏执行
    //
    // TF_INVOKE_CONTINUATION() 宏的定义：
    ...
    // 执行流程：
    //   1. _invoke() 执行完当前任务
    //   2. 调度后继任务，最后一个后继任务缓存到 cache
    //   3. _tear_down_nonasync() 完成清理
    //   4. TF_INVOKE_CONTINUATION() 检查 cache
    //   5. 如果 cache 不为空，跳转到 begin_invoke，继续执行缓存的任务
    //   6. 形成任务链，避免递归调用和栈溢出
    //
    cache = node;
}
...
invoke_task:
    ...
    //
    // 计算公式：
    //   重置值 = num_predecessors() - (node->_nstate & ~NSTATE::MASK)：总前驱数量 - 弱依赖的数量 = 强依赖的数量
    node->_join_counter.fetch_add(node->num_predecessors() - (node->_nstate & ~NSTATE::MASK),
    ...
    // 父节点的 _join_counter 不是用来跟踪父节点自己的前驱依赖，而是用来跟踪它的所有子任务是否完成！
...
inline void Executor::_tear_down_nonasync(Worker& worker, Node* node, Node*& cache) {
    // ========================================================================
    // 【关键】必须先检查 parent，再减少 join_counter
    // ========================================================================
    // 为什么？避免数据竞争（Data Race）
    //
    // 错误的顺序（如果先减 join_counter）：
    ...
    if (auto parent = node->_parent; parent == nullptr) {
        ...
        // 情况 1：顶层任务（没有父节点）
        // ====================================================================
        // 这是 Taskflow 的顶层任务，需要更新 Topology 的 join_counter
        //
        // Topology 的 join_counter 含义：
        //   - 记录当前正在执行或等待执行的任务数量
        //   - 初始值：源节点（无前驱的节点）的数量
        ...
        //
        // fetch_sub(1, std::memory_order_acq_rel) 的含义：
        //   - 原子地减 1，返回减少前的值
        //   - memory_order_acq_rel：获取-释放语义
        //     * 释放（release）：之前的所有写操作对其他线程可见
        //     * 获取（acquire）：之后的所有读操作能看到其他线程的写操作
        //   - 如果返回值为 1，说明减少后变为 0，Topology 完成
        //
        if (node->_topology->_join_counter.fetch_sub(1, std::memory_order_acq_rel) == 1) {
            // ================================================================
            // Topology 完成！所有任务都已执行完毕
            // ================================================================
            // 调用 _tear_down_topology 进行清理和后续处理：
            //   1. 检查是否需要再次运行（run_until 场景）
            //   2. 调用完成回调（_call）
            //   3. 设置 Promise（通知等待的线程）
            //   4. 清理 Topology 对象
            //   5. 如果有下一个 Topology，开始执行
            //
        ...
        // 情况 2：子任务（有父节点）
        // ====================================================================
        // 这是子流（Subflow）或运行时（Runtime）任务的子任务
        // 需要更新父节点的 join_counter
        //
        // 父节点的 join_counter 含义：
        //   - 记录父节点的子任务中还有多少未完成
        //   - 初始值：子图的源节点数量
        ...
        //
        // ====================================================================
        // 【关键】必须在减少 join_counter 之前读取所有需要的数据
        // ====================================================================
        // 为什么？因为 join_counter 变为 0 后，父节点可能被其他线程删除
        //
        // 场景：
        //   1. 当前线程：parent->_join_counter.fetch_sub(1) 返回 1（变为 0）
        //   2. 其他线程：检测到 parent 的 join_counter 为 0，删除 parent
        //   3. 当前线程：访问 parent->_nstate（悬空指针！）
        //
        // 解决方案：先读取 parent->_nstate，保存到局部变量
        //
        auto state = parent->_nstate;  // 提前读取，避免悬空指针访问

        // 原子地减少父节点的 join_counter
        if (parent->_join_counter.fetch_sub(1, std::memory_order_acq_rel) == 1) {
            // ================================================================
            // 所有子任务完成！父节点可以恢复执行
            // ================================================================
            // 检查父节点是否被抢占（PREEMPTED）
            //
            // 什么是抢占（PREEMPTED）？
            ...
            //
            // 为什么检查 PREEMPTED 标志？
            //   - 如果父节点被抢占，需要重新调度它继续执行
            //   - 如果父节点没有被抢占（例如：join() 调用），不需要调度
            //
            if (state & NSTATE::PREEMPTED) {
                // ============================================================
                // 父节点被抢占，需要重新调度
                // ============================================================
                // 使用 _update_cache 将父节点缓存起来
                // 这样可以利用尾调用优化，直接在当前线程继续执行父节点
                //
                // _update_cache 的逻辑：
                //   - 如果 cache 为空：将 parent 缓存到 cache
                //   - 如果 cache 不为空：将 cache 中的任务调度出去，然后缓存 parent
                //
                // 优化效果：
                //   - 避免将父节点放入队列再取出
                //   - 提高缓存局部性（父节点数据可能还在 L1 缓存中）
                //   - 减少上下文切换
                //
                _update_cache(worker, cache, parent);
            }
            // 如果父节点没有被抢占，说明父节点通过 join() 等待子任务完成
            // 此时父节点所在的线程会自己处理后续逻辑，不需要我们调度
        }
    }
}
...
inline void Executor::_invoke_static_task(Worker& worker, Node* node) {
    fprintf(stdout, "[%d] [%d] 执行%s任务\n", getpid(), gettid(), node->name().c_str());
    _observer_prologue(worker, node);
    TF_EXECUTOR_EXCEPTION_HANDLER(worker, node, { std::get_if<Node::Static>(&node->_handle)->work(); });
...
inline bool Executor::_invoke_subflow_task(Worker& worker, Node* node) {
    ...
    if ((node->_nstate & NSTATE::PREEMPTED) == 0) {
        ...
        _observer_prologue(worker, node);
        TF_EXECUTOR_EXCEPTION_HANDLER(worker, node, { h.work(sf); });
        _observer_epilogue(worker, node);
...
inline void Executor::_invoke_multi_condition_task(Worker& worker, Node* node, SmallVector<int>& conds) {
    _observer_prologue(worker, node);
    TF_EXECUTOR_EXCEPTION_HANDLER(worker, node, { conds = std::get_if<Node::MultiCondition>(&node->_handle)->work(); });
    _observer_epilogue(worker, node);
}
...
inline bool Executor::_invoke_async_task(Worker& worker, Node* node) {
    ...
    switch (work.index()) {
        ...
        case 0:
            _observer_prologue(worker, node);
            TF_EXECUTOR_EXCEPTION_HANDLER(worker, node, { std::get_if<0>(&work)->operator()(); });
            _observer_epilogue(worker, node);
            break;
...
inline void Executor::_set_up_topology(Worker* w, Topology* tpg) {
    // if (w == nullptr) {
    //     std::cout << "当前线程对应的工作线程对象为空（因为当前线程是主线程，主线程没有对应的工作线程对象）"
    //               << std::endl;
    // }
    ...
    // 记录当前图中源节点的数量(入度为0节点的数量) 在这里设置topology的初始join_counter!!!
...
I Executor::_set_up_graph(I first, I last, Topology* tpg, Node* parent) {
    // 调试打印，先打印出所有节点的名称 D C B A.
    // for (auto it = first; it != last; ++it) {
    //     std::cout << "节点名称: " << it->get()->_name << std::endl;
    // }
...
// 参数：
//   worker - 执行任务的工作线程
//   tpg    - 刚完成的 Topology 对象
//
inline void Executor::_tear_down_topology(Worker& worker, Topology* tpg) {
    // ========================================================================
    // 获取 Topology 所属的 Taskflow 对象
    // ========================================================================
    // Taskflow 是任务图的定义，Topology 是任务图的运行时实例
    // 一个 Taskflow 可以有多个 Topology（多次运行）
    //
    auto& f = tpg->_taskflow;

    // assert(&tpg == &(f._topologies.front()));
    // 断言：当前 Topology 应该是队列的第一个（正在执行的）

    // ========================================================================
    // 情况 1：需要再次运行 Topology（run_until 场景）
    // ========================================================================
    // 什么是 run_until？
    //   - executor.run_until(taskflow, predicate)
    //   - 重复运行 Taskflow，直到 predicate 返回 true
    //   - 用于循环执行任务图（例如：迭代算法）
    //
    // 判断条件：
    //   1. !tpg->_exception_ptr：没有异常发生
    //   2. !tpg->cancelled()：没有被取消
    //   3. !tpg->_pred()：谓词返回 false（需要继续运行）
    //
    if (!tpg->_exception_ptr && !tpg->cancelled() && !tpg->_pred()) {
        // ====================================================================
        // 需要再次运行，重新设置 Topology
        // ====================================================================
        // assert(tpg->_join_counter == 0);
        // 断言：join_counter 应该为 0（所有任务已完成）

        // 加锁保护 Taskflow 的内部状态
        // 为什么需要加锁？
        //   - 外部线程可能同时调用 executor.run() 添加新的 Topology
        //   - 需要保护 _topologies 队列的并发访问
        //
        std::lock_guard<std::mutex> lock(f._mutex);

        // 重新设置 Topology，准备下一次运行
        // _set_up_topology 的功能：
        //   1. 重置所有节点的 join_counter
        //   2. 调度源节点（无前驱的节点）
        //   3. 设置 Topology 的 join_counter
        //
        _set_up_topology(&worker, tpg);
    }
    // ========================================================================
    // 情况 2：Topology 的最后一次运行（正常完成）
    // ========================================================================
    else {
        // ====================================================================
        // 调用完成回调（如果有）
        // ====================================================================
        // 什么是完成回调？
        //   - executor.run(taskflow).then(callback)
        //   - 在 Topology 完成后调用 callback
        //   - 用于执行清理工作或后续操作
        //
        ...
        // ====================================================================
        // 检查是否有下一个 Topology 等待执行
        // ====================================================================
        // 什么情况下会有多个 Topology？
        //   1. 多次调用 executor.run(taskflow)
        //   2. 第一个 Topology 还在执行时，又提交了新的 Topology
        //   3. Topology 会排队等待执行
        //
        // 使用 unique_lock 而不是 lock_guard：
        //   - 需要在函数中途手动解锁
        //   - unique_lock 支持手动 unlock()
        //
        if (std::unique_lock<std::mutex> lock(f._mutex); f._topologies.size() > 1) {
            ...
            // ================================================================
            // assert(tpg->_join_counter == 0);
            // 断言：当前 Topology 的 join_counter 应该为 0

            // ================================================================
            // 设置 Promise，通知等待的线程
            // ================================================================
            // _carry_out_promise() 的功能：
            //   - 如果有异常：promise.set_exception(exception_ptr)
            //   - 如果正常：promise.set_value()
            //   - 唤醒调用 future.wait() 或 future.get() 的线程
            //
            ...
            // 减少正在运行的 Topology 计数
            // ================================================================
            // _decrement_topology() 的功能：
            //   - 原子地减少 _num_topologies 计数器
            //   - 如果计数器变为 0，通知等待的线程（executor 析构时）
            //
            _decrement_topology();
            ...
            // 设置下一个 Topology，开始执行
            // ================================================================
            // 为什么必须在锁内调用 _set_up_topology？
            //   - 避免与 pop() 操作的内存顺序错误
            //   - 确保 Topology 的状态在设置前是一致的
            //
            _set_up_topology(&worker, tpg);
        } else {
            // ================================================================
            // 没有下一个 Topology，这是最后一个
            // ================================================================
            // assert(f._topologies.size() == 1);
            // 断言：队列中只有当前这一个 Topology

            // ================================================================
            // 移动语义获取 Topology 的所有权
            // ================================================================
            // 为什么使用 std::move？
            //   - Topology 是 unique_ptr，不能拷贝
            //   - 需要在 pop() 之前保存 Topology，避免被销毁
            //   - 延长 Topology 的生命周期，直到 Promise 设置完成
            //
            auto fetched_tpg{std::move(f._topologies.front())};
            f._topologies.pop();

            // 保存 satellite 标志（是否由 executor 管理）
            ...
            // ================================================================
            // 为什么提前解锁？
            //   - _carry_out_promise() 可能触发用户代码
            //   - 用户代码可能调用 executor.run()，需要获取锁
            //   - 避免死锁
            //
            lock.unlock();
            ...
            // 设置 Promise，通知等待的线程
            // ================================================================
            // 【重要】在这之后，Taskflow 的生命周期不再有保证
            //
            // 为什么？
            //   - 调用 promise.set_value() 后，等待的线程被唤醒
            //   - 等待的线程可能立即销毁 Taskflow 对象
            //   - 因此，在这之后不能再访问 Taskflow 的成员
            //
            ...
            // ================================================================
            // 清理由 executor 管理的 Taskflow
            // ================================================================
            // 什么是 satellite？
            //   - executor.run(std::move(taskflow))
            //   - Taskflow 的所有权转移给 executor
            //   - executor 负责在完成后删除 Taskflow
            //
            // 为什么需要这个机制？
            //   - 支持"即发即忘"（fire-and-forget）模式
            //   - 用户不需要保持 Taskflow 对象的生命周期
            //   - executor 自动管理 Taskflow 的生命周期
            //
            if (satellite) {
                // 加锁保护 _taskflows 容器
                std::scoped_lock<std::mutex> satellite_lock(_taskflows_mutex);

                // 从 executor 的 Taskflow 列表中移除
                // 这会触发 Taskflow 的析构函数，释放资源
                _taskflows.erase(*satellite);
            }

            // TODO: 未来可能需要在 wait() 上同步
            // （这意味着上面的代码应该移到 set_value 之前）
        }
    }
}
